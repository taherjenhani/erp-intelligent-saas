...
- Configure `EMAIL_OUTBOX_LOCK_HEARTBEAT_MS` lower than `EMAIL_OUTBOX_LOCK_TIMEOUT_MS` so slow SMTP deliveries do not get recovered by another worker.
- Enable `TOKEN_CLEANUP_WORKER_ENABLED=true` or schedule `npm run tokens:cleanup` externally.
